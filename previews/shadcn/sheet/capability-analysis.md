...
| 本地 | shadcn | zardui | 备注 |
|------|--------|--------|------|
| `sheet` | `@shadcn/sheet` | `sheet` | 本地 sheet 对标 shadcn/sheet 与 zardui/sheet。 |
...
|------|------|------|--------|--------|---------------------|
| API 覆盖度 | 20 | 3.0 | 3.5 | 5.0 | E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1 |
| 交互与状态语义 | 15 | 4.0 | 2.0 | 3.5 | E-LOCAL-2, E-SHADCN-2, E-ZARDUI-2 |
| 组合与扩展能力 | 15 | 4.0 | 3.5 | 5.0 | E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1 |
| 可访问性 | 15 | 4.0 | 2.0 | 4.0 | E-LOCAL-2, E-SHADCN-2, E-ZARDUI-2 |
...
| **加权总分** | **100** | **75.0** | **48.5** | **85.0** |  |

## 3. Top 3 实现差距

### Gap-1: 本地 API 覆盖与对标实现存在差距
- 影响等级：高
- 差距陈述：本地在 API 覆盖维度与对标实现存在差距（相对 shadcn 差值 -0.5，相对 zardui (sheet) 差值 -2.0）。当前能力点分布在多个输入/子节点约定中，常见场景下缺少一条“最短实现路径”。
- 证据：E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1
- 本地改造建议：
  - 修改文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/sheet/sheet.component.ts`
...
- 本地改造建议：
  - 新增文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/sheet/sheet.component.spec.ts`
...
### E-LOCAL-1
- 实现方：local
- 文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/sheet/sheet.component.ts`
- 行号：`L44-L61`
- 结论标签：API/结构
```ts
...
### E-LOCAL-2
- 实现方：local
- 文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/sheet/index.ts`
- 行号：`L1-L16`
- 结论标签：状态/可访问性
```ts
export {
...
export type { SheetSide, SheetSize } from './sheet.component';
```

### E-SHADCN-1
- 实现方：shadcn
- 文件：`/tmp/shadcn-ui/apps/v4/registry/new-york-v4/ui/sheet.tsx`
- 行号：`L7-L24`
- 结论标签：API/变体
```tsx
...
### E-SHADCN-2
- 实现方：shadcn
- 文件：`/tmp/shadcn-ui/apps/v4/registry/new-york-v4/examples/sheet-demo.tsx`
- 行号：`L17-L34`
- 结论标签：语义/用法
```tsx
    <Sheet>
...
1. [ ] 在 `/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/sheet/sheet.component.ts` 收敛 API 输入并补齐缺失能力（含默认值和类型）。
2. [ ] 统一根组件状态机与 a11y 契约（role/aria/keyboard path）并补文档示例。
3. [ ] 新增 `/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/sheet/sheet.component.spec.ts`，覆盖关键行为回归。
