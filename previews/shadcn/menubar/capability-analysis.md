...
| 本地 | shadcn | zardui | 备注 |
|------|--------|--------|------|
| `menubar` | `@shadcn/menubar` | `menu` | 本地 menubar 对标 shadcn/menubar 与 zardui/menu。 |
...
| 交互与状态语义 | 15 | 5.0 | 2.0 | 4.0 | E-LOCAL-2, E-SHADCN-2, E-ZARDUI-2 |
| 组合与扩展能力 | 15 | 5.0 | 5.0 | 3.0 | E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1 |
| 可访问性 | 15 | 4.0 | 2.0 | 3.5 | E-LOCAL-2, E-SHADCN-2, E-ZARDUI-2 |
...
| **加权总分** | **100** | **87.0** | **59.0** | **84.0** |  |

## 3. Top 3 实现差距

### Gap-1: 本地 API 覆盖与对标实现存在差距
- 影响等级：高
- 差距陈述：本地在 API 覆盖维度与对标实现存在差距（相对 shadcn 差值 -0.5，相对 zardui (menu) 差值 -0.5）。当前能力点分布在多个输入/子节点约定中，常见场景下缺少一条“最短实现路径”。
- 证据：E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1
- 本地改造建议：
  - 修改文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/menubar/menubar.component.ts`
...
- 影响等级：高
- 差距陈述：当前本地测试文件数量为 0，难以为 API 扩展和行为重构提供足够回归保护。对标 zardui 该映射下有 6 个测试文件，稳定性基线更高。
- 证据：E-LOCAL-1, E-ZARDUI-2
- 本地改造建议：
  - 新增文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/menubar/menubar.component.spec.ts`
...
### E-LOCAL-1
- 实现方：local
- 文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/menubar/menubar.component.ts`
- 行号：`L477-L494`
- 结论标签：API/结构
```ts
// ============================================================================
...
### E-LOCAL-2
- 实现方：local
- 文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/menubar/index.ts`
- 行号：`L1-L18`
- 结论标签：状态/可访问性
```ts
export {
...
### E-SHADCN-1
- 实现方：shadcn
- 文件：`/tmp/shadcn-ui/apps/v4/registry/new-york-v4/ui/menubar.tsx`
- 行号：`L7-L24`
- 结论标签：API/变体
```tsx
...
### E-SHADCN-2
- 实现方：shadcn
- 文件：`/tmp/shadcn-ui/apps/v4/registry/new-york-v4/examples/menubar-demo.tsx`
- 行号：`L17-L34`
- 结论标签：语义/用法
```tsx
...
### E-ZARDUI-1
- 实现方：zardui
- 文件：`/tmp/zardui/libs/zard/src/lib/shared/components/menu/demo/menu.ts`
- 行号：`L1-L18`
- 结论标签：API/模板
```ts
...
### E-ZARDUI-2
- 实现方：zardui
- 文件：`/tmp/zardui/libs/zard/src/lib/shared/components/menu/menu-label.component.spec.ts`
- 行号：`L1-L18`
- 结论标签：交互/测试
```ts
...
1. [ ] 在 `/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/menubar/menubar.component.ts` 收敛 API 输入并补齐缺失能力（含默认值和类型）。
2. [ ] 统一根组件状态机与 a11y 契约（role/aria/keyboard path）并补文档示例。
3. [ ] 新增 `/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/menubar/menubar.component.spec.ts`，覆盖关键行为回归。
