...
| 本地 | shadcn | zardui | 备注 |
|------|--------|--------|------|
| `kbd` | `@shadcn/kbd` | `kbd` | 本地 kbd 对标 shadcn/kbd 与 zardui/kbd。 |
...
|------|------|------|--------|--------|---------------------|
| API 覆盖度 | 20 | 2.0 | 2.5 | 3.0 | E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1 |
| 交互与状态语义 | 15 | 2.0 | 2.0 | 2.0 | E-LOCAL-2, E-SHADCN-2, E-ZARDUI-2 |
| 组合与扩展能力 | 15 | 2.5 | 2.5 | 2.5 | E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1 |
| 可访问性 | 15 | 2.0 | 2.0 | 2.0 | E-LOCAL-2, E-SHADCN-2, E-ZARDUI-2 |
| 类型安全与开发体验 | 10 | 2.5 | 2.0 | 2.5 | E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1 |
| 性能与资源管理 | 15 | 2.5 | 2.0 | 3.5 | E-LOCAL-2, E-SHADCN-1, E-ZARDUI-1 |
| 测试与稳定性证据 | 10 | 1.0 | 1.0 | 3.5 | E-LOCAL-1, E-ZARDUI-2 |
| **加权总分** | **100** | **42.0** | **41.5** | **54.0** |  |

## 3. Top 3 实现差距

### Gap-1: 本地 API 覆盖与对标实现存在差距
- 影响等级：高
- 差距陈述：本地在 API 覆盖维度与对标实现存在差距（相对 shadcn 差值 -0.5，相对 zardui (kbd) 差值 -1.0）。当前能力点分布在多个输入/子节点约定中，常见场景下缺少一条“最短实现路径”。
- 证据：E-LOCAL-1, E-SHADCN-1, E-ZARDUI-1
- 本地改造建议：
  - 修改文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/kbd/index.ts`
...
- 本地改造建议：
  - 新增文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/kbd/kbd.component.spec.ts`
...
### E-LOCAL-1
- 实现方：local
- 文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/kbd/index.ts`
- 行号：`L1-L2`
- 结论标签：API/结构
```ts
...
### E-LOCAL-2
- 实现方：local
- 文件：`/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/kbd/kbd.directive.ts`
- 行号：`L27-L44`
- 结论标签：状态/可访问性
```ts
...
### E-SHADCN-1
- 实现方：shadcn
- 文件：`/tmp/shadcn-ui/apps/v4/registry/new-york-v4/ui/kbd.tsx`
- 行号：`L1-L18`
- 结论标签：API/变体
```tsx
...
### E-SHADCN-2
- 实现方：shadcn
- 文件：`/tmp/shadcn-ui/apps/v4/registry/new-york-v4/examples/kbd-button.tsx`
- 行号：`L5-L16`
- 结论标签：语义/用法
```tsx
  return (
...
### E-ZARDUI-1
- 实现方：zardui
- 文件：`/tmp/zardui/libs/zard/src/lib/shared/components/kbd/kbd.component.ts`
- 行号：`L7-L24`
- 结论标签：API/模板
```ts
...
### E-ZARDUI-2
- 实现方：zardui
- 文件：`/tmp/zardui/libs/zard/src/lib/shared/components/kbd/demo/kbd.ts`
- 行号：`L1-L18`
- 结论标签：交互/测试
```ts
...
1. [ ] 在 `/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/kbd/index.ts` 收敛 API 输入并补齐缺失能力（含默认值和类型）。
2. [ ] 统一根组件状态机与 a11y 契约（role/aria/keyboard path）并补文档示例。
3. [ ] 新增 `/Users/wanyaozhong/Projects/argusx-f/src/app/shared/ui/kbd/kbd.component.spec.ts`，覆盖关键行为回归。
